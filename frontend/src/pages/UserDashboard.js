...
import { FiBook, FiBarChart2, FiAward, FiClock } from 'react-icons/fi';
            ...
            <div>
                ...
                {loading ? (
                    ...
                    <div className="card p-12 text-center">
                        ...
                        <p className="text-gray-400 dark:text-slate-600 text-sm mt-1">Check back later for new quizzes</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                        {quizzes.map(quiz => (
                            <div key={quiz.id} className="card p-6 flex flex-col hover:shadow-md transition-shadow">
                                <div className="flex-1">
                                    <h3 className="text-base font-semibold text-gray-800 dark:text-dark-text mb-2">{quiz.title}</h3>
                                    ...
                                    <div className="space-y-1.5 mb-4 text-sm text-gray-500 dark:text-dark-muted">
                                        {quiz.timeLimit && (
                                            <p className="flex items-center gap-1.5"><FiClock className="text-primary-500" /> {quiz.timeLimit} minutes</p>
                                        )}
